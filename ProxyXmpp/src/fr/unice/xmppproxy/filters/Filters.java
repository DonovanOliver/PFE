            ...
            //if (Config.contains("filters")) 
        	{
                ...
                for (Element node : nodes) {
                    ...
                    if (type.equalsIgnoreCase("internal")) {
                        ...
                        if (both || type.equalsIgnoreCase("client")) 
                        	{
                         	Log.i("myapps","Client Filters Node : " + filter.name);
                            
                        	clientFilters.add(filter);
                        	}
                        if (both || type.equalsIgnoreCase("server")) 
                        	{
                        	Log.i("myapps","Server Filters Node : " + filter.name);
                        	serverFilters.add(filter);
    ...
    private static void addInternalFilter(Element node) {
        ...
        if (nodeName.equalsIgnoreCase("google_plain_mechanism")) {
            if (node.getChild("enabled")!= null) {
            	Log.i("myapps","Server Filters Node : " + nodeName);
            	clientFilters.add((new Credentials()));
                serverFilters.add((new Credentials()));
            }
        }
        if (nodeName.equalsIgnoreCase("google_secure_message")) {
            if (node.getChild("enabled")!= null) {
            	Log.i("myapps","Server Filters Node : " + nodeName);
            	clientFilters.add((new SecureMessage()));
                serverFilters.add((new SecureMessage()));
            }
        }
    }
