...
import fr.unice.xmppproxy.filters.Filter;
	     ...
	     try {
	    	 ...
	    	 Log.d("myapps"," SecurityPreferences ReadSecuPref Start");	
	    	 {
	                ...
	                for (Element node : nodes) 
	                {
	                    ...
	                    if (beginner != null) setUserLevel(1);
	                    String intermediate = node.getChildTextTrim("intermediate");
	                    if (intermediate != null) setUserLevel(2);
	                    String advanced = node.getChildTextTrim("advanced");
	                    if (advanced != null) setUserLevel(3);
	                    
	                    Log.i("myapps",""+" beginner : "+beginner+" intermediate : "+intermediate+" advanced : "+advanced);
	                    
	                    
	                    /*Beginner*/
	                    String type = node.getChildTextTrim("type");
	                    if (type != null) setDataType(type);
	                    String level = node.getChildTextTrim("level");
	                    if (level != null) setSecurityLevel(Integer.parseInt(level.substring(level.length()-1)));
	                    
	                    /*intermediate & Advanced */
	                    
	                    String Conf = node.getChildTextTrim("confidentiality");
                        if (Conf != null) setConfidentiality(Boolean.valueOf(Conf));
                        String ConfAlgo = node.getChildTextTrim("ConfAlgo");
                        if (ConfAlgo != null) setConfidentialityAlgorithm(ConfAlgo);
                        String authenticity = node.getChildTextTrim("authenticity");
                        if (authenticity != null) setAuthenticity(Boolean.valueOf(authenticity));
                        String authAlgo = node.getChildTextTrim("authAlgo");
                        if (authAlgo != null) setAuthenticityAlgorithm(authAlgo);
                        String integrity = node.getChildTextTrim("integrity");
                        if (integrity != null) setIntegrity(Boolean.valueOf(authenticity));
                        String nonrepudiation = node.getChildTextTrim("non-repudiation");
                        if (nonrepudiation != null) setNonrepudiation((Boolean.valueOf(nonrepudiation)));
                        Log.i("myapps"," type : "+type+" level : \n"+level+" Conf : "+Conf+" Auth : "+authenticity+" inter "+integrity+"\nAuthalgo : "+authAlgo+" ConfAlgo : "+ConfAlgo);
	                }
	                    /*
Advanced 

xml.setValue("confidentiality", ""+chkConfidentiality.isChecked());
xml.setValue("ConfAlgo", ""+spinnerConfidentialityAlgorithms.getSelectedItem().toString());
xml.setValue("authenticity", ""+chkAuthenticity.isChecked());
xml.setValue("authAlgo", ""+spinnerAuthenticityAlgorithms.getSelectedItem().toString());
